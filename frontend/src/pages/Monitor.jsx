...
const Monitor = () => {
    ...
    // Load all dashboards and restore selected from localStorage
    useEffect(() => {
        const fetchDashboards = async () => {
            try {
                ...
                // Restore previously selected dashboard from localStorage
                const savedId = localStorage.getItem('lastSelectedDashboardId');
                if (all.length > 0) {
                    let saved = null;
                    if (savedId) {
                        saved = all.find(d => d._id === savedId);
                    }
                    if (saved) {
                        setSelectedDashboard(saved);
                    } else {
                        // If no saved selection or it no longer exists, default to the first dashboard
                        setSelectedDashboard(all[0]);
                    }
                } else {
                    // Only redirect if there are absolutely no dashboards available
    ...
    // Emergency alert polling via localStorage
    useEffect(() => {
    ...
    // Per-dashboard telemetry socket + polling
    useEffect(() => {
        if (!selectedDashboard) return;
        const socket = io(import.meta.env.VITE_API_URL);
        ...
        socket.on('device-data', (data) => {
            if (data.deviceId === selectedDashboard.deviceId) {
                ...
                const filteredData = {
                    _id: data._id, deviceId: data.deviceId,
                    timestamp: data.timestamp, createdAt: data.createdAt,
                    updatedAt: data.updatedAt, action: data.action,
                    warningLevel: data.warningLevel, accidentDetected: data.accidentDetected,
                    speed: data.speed, batterySOH: data.batterySOH,
                };
        ...
        const fetchData = async () => {
            try {
                ...
                if (telemetryRes.data.length > 0) setLatestData(telemetryRes.data[0]);
            } catch (error) {
    ...
    const kpis = [
        {
            icon: <Bike size={24} />,
            label: 'Battery SOC', value: hasSOC ? Number(latestData.batterySOC).toFixed(1) : '---', unit: '%',
        ...
        {
            icon: <Shield size={24} />,
            label: 'Battery SOH', value: hasSOH ? Number(latestData.batterySOH).toFixed(1) : '---', unit: '%',
            grad: hasSOH ? 'grad-emerald' : 'grad-dark border border-white/5',
            sub: hasSOH ? 'Health indicator' : '---',
        },
        {
            icon: <Zap size={24} />,
            label: 'Voltage', value: hasVoltage ? `${latestData.batteryVoltage}` : '---', unit: 'V',
    ...
    // Show a loading/redirect state while dashboards are being fetched
    if (!selectedDashboard) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6">
                <div className="w-12 h-12 border-4 border-slate-100 border-t-[#10B981] rounded-full animate-spin"></div>
                <p className="text-[#94A3B8] text-sm font-bold uppercase tracking-widest">Loading Telemetry...</p>
            </div>
        );
    }
                    ...
                    if (features.includes('ignitionSwitch')) {
                        ...
                        cards.push(
                            <div key="ignitionSwitch" className={`modern-kpi border relative overflow-hidden flex flex-col items-center justify-center transition-all duration-500
                                ...
                                <div className={`absolute inset-0 rounded-[inherit] transition-opacity duration-700 pointer-events-none ${isOn ? 'opacity-100' : 'opacity-0'}`}
                                ...
                                <div className="relative flex flex-col items-center justify-center gap-5 py-4">
                                    <button
                                        ...
                                        className={`relative w-24 h-24 rounded-full flex items-center justify-center transition-all duration-500 focus:outline-none
                                            ${ignitionLoading ? 'cursor-wait opacity-60' : !hasIgnition ? 'cursor-not-allowed opacity-30' : 'hover:scale-105 active:scale-95'}`}
                                        style={{
                                            background: isOn
                                    ...
                                    >
                                        {ignitionLoading && (<span className="absolute inset-0 rounded-full border-4 border-transparent border-t-emerald-400 animate-spin" />)}
                                        ...
                                        {isOn && !ignitionLoading && (<span className="absolute inset-0 rounded-full animate-ping opacity-20" style={{ background: 'rgba(16,185,129,0.4)' }} />)}
                                    </button>
                                    <div className="text-center">
                                        ...
                                        <div className={`mt-2 inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest transition-all duration-500
                                            ${!hasIgnition ? 'border-white/10 text-white/20 bg-white/5' : isOn ? 'border-emerald-500/40 text-emerald-400 bg-emerald-500/10' : 'border-red-500/30 text-red-400 bg-red-500/10'}`}>
                    ...
                    if (features.includes('batteryTemperature')) {
                        ...
                        cards.push(
                            <div key="batteryTemperature" className="modern-kpi grad-dark border border-white/5 relative overflow-hidden group">
                                <div className="modern-icon-box bg-white/10 text-white"><Thermometer size={20} /></div>
                                ...
                                <div className="relative z-10 flex flex-col items-center justify-center w-full h-full text-center p-6">
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p className={`text-xl font-black uppercase tracking-tight mt-0.5 ${statusTextColor}`}>{statusText}</p>
                                            <p className={`text-[9px] font-bold uppercase tracking-widest mt-1 opacity-60 ${subTextColor}`}>{subText}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    if (features.includes('motorRPM')) {
                        ...
                        cards.push(
                            <div key="motorRPM" className="modern-kpi grad-dark border border-white/5 relative overflow-hidden group">
                                <div className="modern-icon-box bg-white/10 text-white"><Gauge size={20} /></div>
                                <div className="mt-auto relative z-10">
                                    <div className="flex items-baseline gap-1">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="modern-chart-card w-full">
                            ...
                            <div className="h-[260px] -ml-4">
                                <Chart options={barOptions} series={socSeries} type="bar" height="100%" width="100%" />
                            </div>
                        </div>
                    </div>
                )}
                                        ...
                                        {hasGPS ? (
                                            <MapContainer key={selectedDashboard?.deviceId} center={mapCenter} zoom={15} style={{ height: '100%', width: '100%', minHeight: '300px' }} zoomControl={false}>
                                                ...
                                                <Marker position={[lat, lng]}>
                                                    <Popup>
                                                        <strong>E-Vehicle Location</strong><br />
