        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            {isSelectingDashboard ? (
                ...
                <>
                    ...
                    {/* Device History Buffer - Detailed Telemetry Logs */}
                    <div className="space-y-6 mt-12">
                        <div className="flex justify-between items-center px-2">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl bg-[#064E3B] flex items-center justify-center text-white">
                                    <Clock size={20} />
                                </div>
                                <div>
                                    <h2 className="text-xl font-black text-[#064E3B] tracking-tight">Device History Buffer</h2>
                                    <p className="text-[10px] font-black text-[#94A3B8] uppercase tracking-[0.2em] mt-0.5">Comprehensive Telemetry Stream</p>
                                </div>
                            </div>
                            <div className="flex gap-2">
                                <div className="bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100 flex items-center gap-2">
                                    <div className="w-1.5 h-1.5 rounded-full bg-[#10B981] animate-pulse"></div>
                                    <span className="text-[9px] font-black uppercase tracking-widest text-[#10B981]">{deviceData.length} Records In Buffer</span>
                                </div>
                            </div>
                        </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-[#F0FDF4]">
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-[#064E3B]/60">Timestamp</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-[#064E3B]/60">SOC</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-[#064E3B]/60">Voltage</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-[#064E3B]/60">Speed</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-[#064E3B]/60">Brake</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-[#064E3B]/60">Status</th>
                                            <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-[#064E3B]/60 text-right">Event</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-[#D1FAE5]">
                                        {deviceData.map((d, idx) => (
                                            <tr key={d._id || idx} className="hover:bg-emerald-50/30 transition-colors">
                                                <td className="px-6 py-4">
                                                    <div className="flex flex-col">
                                                        <span className="text-xs font-bold text-[#064E3B]">{new Date(d.timestamp).toLocaleTimeString()}</span>
                                                        <span className="text-[9px] font-medium text-[#94A3B8]">{new Date(d.timestamp).toLocaleDateString()}</span>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className={`text-xs font-black ${d.batterySOC < 20 ? 'text-rose-500' : 'text-[#064E3B]'}`}>{d.batterySOC}%</span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className="text-xs font-bold text-[#6B7280]">{d.batteryVoltage}V</span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className="text-xs font-bold text-[#064E3B]">{d.speed || 0} km/h</span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-md ${d.brakeStatus === 'APPLIED' ? 'bg-rose-100 text-rose-600 border border-rose-200' : 'bg-emerald-100 text-emerald-600 border border-emerald-200'}`}>
                                                        {d.brakeStatus}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className={`w-2 h-2 rounded-full inline-block mr-2 ${d.accidentDetected ? 'bg-rose-500 animate-pulse' : 'bg-[#10B981]'}`}></span>
                                                    <span className="text-[10px] font-bold text-[#6B7280] uppercase tracking-wider">{d.accidentDetected ? 'Alert' : 'Normal'}</span>
                                                </td>
                                                <td className="px-6 py-4 text-right">
                                                    <span className="text-[10px] font-mono text-[#94A3B8]">{d.action || 'SYNC'}</span>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
